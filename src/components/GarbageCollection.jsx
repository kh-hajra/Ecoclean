...
const GarbageCollection = () => {
  const packageDescriptions = {
    basic: "Ideal for light garbage collection tasks, such as small residential areas or light commercial spaces.",
    standard: "Best for regular garbage collection needs, suitable for medium-sized residential areas or commercial spaces.",
    premium: "Recommended for thorough or heavy-duty garbage collection, such as large residential complexes, industrial areas, or event clean-ups.",
  };
 ...
 const [isBookingSummaryOpen, setIsBookingSummaryOpen] = useState(false);
  ...
  useEffect(() => {
    const fetchServiceDetails = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/services/garbage-collection');
...
<div className="absolute z-10 text-center px-8 max-w-4xl mt-2">
  ...
  <h1 className="text-5xl sm:text-6xl lg:text-8xl font-bold tracking-tight leading-tight font-serif" 
    style={{ fontFamily: 'Rische, serif', whiteSpace: 'nowrap' }}>
  <span className="text-white">Garbage Coll</span>
  <span className="text-black">ect</span>
  <span className="text-white">ion</span>
</h1>

  <p className="text-lg sm:text-xl lg:text-2xl text-gray-200 pt-5 max-w-2xl mx-auto">
    <span className="text-black">Efficient</span> waste collection <span className="text-black">solutions</span> for cleaner streets and <span className="text-black">greener communities.</span>
  </p>
